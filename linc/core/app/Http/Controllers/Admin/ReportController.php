...
class ReportController extends Controller
{
    public function transaction(Request $request)
    {
        ...
        $transactions = Transaction::with('user', 'agent')->orderBy('id', 'desc');
        if ($request->search) {
    ...
    {
        $loginLogs = UserLogin::orderBy('id', 'desc')->with('user', 'agent');
    ...
    public function notificationHistory(Request $request)
    {
        ...
        if ($search) {
            $logs = $logs->where(function ($q) use ($search) {
                return $q->whereHas('user', function ($user) use ($search) {
                    $user->where('username', 'like', "%$search%");
                })->orWhereHas('agent', function ($agent) use ($search) {
                    $agent->where('username', 'like', "%$search%");
                });
        ...
        $logs = $logs->with('user', 'agent')->paginate(getPaginate());
