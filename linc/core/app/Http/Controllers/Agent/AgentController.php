...
class AgentController extends Controller
{
    ...
    public function dashboard($duration = 'today')
    {
        ...
        $transactions = Transaction::filterAgent()->filterByDay(365)
            ->where('trx_type', '+')
        ...
        $insights['sent_amounts'] = SendMoney::filterAgent()->whereIn('status', [1, 2])->filterByDay($day)->sum('base_currency_amount');
        $insights['deposits']     = Deposit::filterAgent()->where('status', 1)->filterByDay($day)->sum('amount');
        ...
        return view('agent.dashboard', compact('pageTitle', 'widget', 'report', 'insights', 'transactions'));
    }
    ...
    public function transactions(Request $request)
    {
        ...
        $transactions = Transaction::filterAgent()
            ->searchable('trx', $request->trx, true)
            ->searchable('trx_type', $request->type == 'minus_trx' ? '-' : ($request->type == 'plus_trx' ? '+' : null))
    ...
    public function depositHistory(Request $request)
    {
        ...
        $deposits  = Deposit::where('status', '!=', 0)->filterAgent()->searchable('trx', request()->trx);
